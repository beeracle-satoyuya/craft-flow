...
$convertToZenkinFormat = function () {
    ...
    try {
        ...
        $filePath = 'bank-transfers/' . $userId . '/' . $filename;

        // ディレクトリが存在しない場合は作成
        $directory = 'bank-transfers/' . $userId;
        if (!Storage::disk('local')->exists($directory)) {
            Storage::disk('local')->makeDirectory($directory);
        }

        // 永続保存ディレクトリに保存
        Storage::disk('local')->put($filePath, $zenkinContent);

        // 履歴をデータベースに保存
        $conversion = BankTransferConversion::create([
            'user_id' => $userId,
            'original_filename' => $this->originalFilename ?? 'unknown.xlsx',
...
$getDownloadUrl = function () {
    ...
    return route('bank-transfers.download-history', ['conversion' => $this->conversionId]);
};
